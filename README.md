...
팀원으로서 냉장고 도메인의 핵심 기능 구현을 담당했습니다. 단순 CRUD 중심의 재료 관리 기능을 넘어, 공공데이터 기반 레시피 수집·정규화, 영수증 OCR 텍스트 파싱, 유통기한 예측 모델, 재료 소진 중심 추천 알고리즘, OpenAI API 기반 제철요리 추천까지 이어지는 데이터 기반 추천 흐름을 설계하고 구현했습니다.

제가 맡은 주요 구현 범위는 다음과 같습니다.

- 공공데이터포털에서 레시피 기본정보, 상세정보, 재료 데이터를 수집하고, 추천 알고리즘에서 활용할 수 있도록 재료명·용량·조리정보를 파싱 및 정규화했습니다. 원천 데이터의 표기 불일치와 중복 데이터를 정제해, 레시피-재료 매칭 정확도를 높일 수 있는 형태로 데이터 구조를 재가공했습니다.
- Google Vision API로 추출한 영수증 OCR 텍스트를 후처리하는 로직을 구현했습니다. OCR 결과에서 상품명, 수량, 불필요한 결제 정보 등을 분리하고, 사용자가 인식 결과를 검수·수정한 뒤 냉장고 재료로 저장할 수 있는 화면을 구성했습니다. 이를 통해 비정형 OCR 텍스트를 실제 서비스 데이터로 전환하는 흐름을 만들었습니다.
- 식품분류, 품목명, 보관조건 데이터를 기반으로 Random Forest 회귀 모델을 설계해 재료별 예상 유통기한을 예측했습니다. 범주형 데이터를 인코딩하고, 품목 특성과 보관 방식에 따른 유통기한 차이를 학습하도록 구성했습니다. 학습 데이터에 존재하지 않는 신규 품목에 대해서는 품목명·분류 유사도를 기반으로 가장 가까운 식품군을 탐색하는 폴백 로직을 적용해 예측 실패 가능성을 줄였습니다.
- 보유 식재료와 레시피 재료 간의 유사도를 계산하는 추천 모델을 설계했습니다. 재료명을 벡터화한 뒤 코사인 유사도를 활용해 사용자의 냉장고 재료와 레시피 재료 간 일치도를 산출하고, 보유 재료를 최대한 활용할 수 있는 레시피를 우선 추천하도록 구성했습니다. 이를 통해 단순 키워드 포함 여부가 아니라 재료 구성의 유사성을 기준으로 추천 품질을 개선했습니다.
- 실서비스 추천 API에는 주재료, 부재료, 양념의 역할별 중요도를 반영한 가중치 기반 매칭 스코어를 적용했습니다. 모든 재료를 동일하게 비교하지 않고, 레시피 완성도에 더 큰 영향을 주는 주재료에는 높은 가중치를 부여하고, 양념류에는 낮은 가중치를 적용했습니다. 이를 통해 사용자가 실제로 만들 수 있는 가능성이 높은 레시피가 상위에 노출되도록 추천 라우터를 설계했습니다.
- OpenAI API를 활용해 식비 절약 관점의 제철요리 추천 전문가 기능을 구현했습니다. 사용자의 보유 재료, 계절성, 예산 절감 목적을 프롬프트에 반영하고, 응답은 JSON Schema 기반 구조로 제한해 후처리 안정성을 확보했습니다. 이를 통해 생성형 AI 응답을 단순 텍스트 출력이 아니라 서비스 로직에서 재사용 가능한 정형 데이터로 변환했습니다.
...
팀원으로서 냉장고 도메인의 핵심 기능 구현을 담당했습니다. 단순히 재료를 등록하고 조회하는 기능이 아니라, 공공데이터포털 레시피 데이터 수집·정제, 영수증 OCR 텍스트 파싱, 유통기한 예측, 재료 소진 레시피 추천, OpenAI API 기반 제철요리 추천까지 이어지는 전체 흐름을 구현했습니다.

제가 맡은 주요 구현 범위는 다음과 같습니다.

- 공공데이터포털에서 레시피 기본정보, 상세정보, 재료 데이터를 가져와 앱 추천 엔진에서 사용할 수 있도록 파싱 및 정규화했습니다.
- Google Vision API로 추출한 영수증 OCR 텍스트를 정제하고, 사용자가 인식 결과를 확인·수정한 뒤 냉장고 재료로 저장하는 화면을 구현했습니다.
- 식품분류, 품목명, 보관조건 데이터를 기반으로 Random Forest 유통기한 예측 모델을 설계하고, 신규 품목에 대한 유사도 기반 폴백 로직을 구성했습니다.
- 보유 식재료와 레시피 재료 간 코사인 유사도를 계산해, 냉장고 속 재료를 최대한 소진할 수 있는 레시피 추천 모델을 설계했습니다.
- 실서비스 API에는 주재료, 부재료, 양념의 역할별 가중치를 반영한 매칭 스코어 기반 추천 라우터를 연동했습니다.
- OpenAI API를 연결해 식비 절약 관점의 제철요리 추천 전문가 기능을 구현하고, JSON Schema 기반 응답 형식으로 후처리 안정성을 확보했습니다.
